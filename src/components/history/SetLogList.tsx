...
export function SetLogList({ entries, onDelete }: SetLogListProps) {
  ...
  return (
    <div className="mb-4">
      <div className="text-[10px] text-dim font-semibold tracking-wider mb-2">SET LOG HISTORY</div>
      {Array.from(groups.values()).map((group) => (
        <div key={group.label} className="mb-3">
          <div className="text-[10px] text-muted font-bold mb-1">{group.label}</div>
          <div className="space-y-0.5">
            {group.entries.map((entry) => {
              ...
              return (
                <div
                  key={entry.id}
                  className="flex items-center justify-between py-1 px-2 bg-card rounded text-[11px]"
                >
                  ...
                  {confirmId === entry.id ? (
                    <div className="flex gap-1">
                      <button
                        ...
                        className="text-[10px] text-danger bg-transparent border border-danger rounded px-1.5 py-0.5 cursor-pointer"
                      >
                        Delete
                      </button>
                      <button
